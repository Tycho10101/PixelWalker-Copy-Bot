...
export default defineComponent({
  setup() {
    ...
    let pwGameClient: PWGameClient
    let pwApiClient: PWApiClient
    ...
    async function onConnectButtonClick() {
      ...
      loading.value = true
      pwApiClient = new PWApiClient(email.value, password.value)
      ...
      await pwApiClient.authenticate()
      pwGameClient = await pwApiClient.joinWorld(worldId.value, {
        gameSettings: {
          handlePackets: ['PING'],
        },
      })
    ...
    async function onDisconnectButtonClick() {
      pwGameClient.value?.socket?.close()
    }
